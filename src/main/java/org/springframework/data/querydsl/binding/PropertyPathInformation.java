...
class PropertyPathInformation implements PathInformation {
  ...
  private static Path<?> reifyPath(EntityPathResolver resolver, PropertyPath path, Optional<Path<?>> base) {
    ...
    return map.orElseGet(() -> {
      ...
      if (field == null) {
        try {
          entityPath = resolver.createSubPath(path.getOwningType().getType(), entityPath.toString());
          field = ReflectionUtils.findField(entityPath.getClass(), path.getSegment());
        } catch (Exception e) {
        }
      }

      if (field == null) {
        throw new IllegalArgumentException(String.format("Unable to find field %s on %s!", path.getSegment(), entityPath.getClass()));
      }
